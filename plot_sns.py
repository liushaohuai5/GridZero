...
axs[0, 0].plot(steps, max_gen[settings.renewable_ids, :].sum(0), color=colors[2], linestyle='dotted',
         label='Renewable overestimated prediction')
...
axs[0, 0].fill_between(steps_1, max_gen_p, min_gen_p, color=colors[2], alpha=0.3, label='DAS adjust capacity area')
axs[0, 0].plot(steps_1, data[:, 1], label='DAS renewable consumption', color=colors[2], linestyle='dashdot')
...
axs[0, 1].plot(steps, ori_max_gen[settings.renewable_ids].sum(0), color=colors[0], linestyle='dotted',
         label='Renewable underestimated prediction')
...
axs[0, 1].fill_between(steps_1, max_gen_p, min_gen_p, color=colors[0], alpha=0.3,
                       # label='DAS adjust capacity area'
                       )
axs[0, 1].fill_between(steps_1, data1[:, 2], data[:, 1], color='mediumseagreen', alpha=0.3)
axs[0, 1].plot(steps_1, data[:, 1],
               # label='DAS renewable consumption',
               color=colors[0], linestyle='dashdot')
...
axs[1, 1].plot(steps, g_data[:, 1], color=colors[4], linestyle='dashdot', label='GridZero renewable consumption')
...
axs[1, 0].plot(steps, sac_data[:, 1], color=colors[1], linestyle='dashdot', label='SAC renewable consumption')
...
g_curtail = g_data[:, 2] - g_data[:, 1]
under_curtail = data1[:, 2] - data[:, 1]
import ipdb
ipdb.set_trace()
...
axs[0, 0].set_title('(A1) DAS: Retrofit level = 0 %')
...
axs[0, 1].set_title('(A2) DAS: Retrofit level = 10 %')
...
axs[1, 0].set_title('(A3) DAS: Retrofit level = 20 %')
...
axs[1, 1].set_title('(B) RTS-GridZero: Retrofit level = 0 %')
fig.supxlabel('Steps', fontsize=20)
fig.supylabel('Power', fontsize=20)
# fig.legend(loc='lower right', fontsize=12)
fig.legend(
...
plt.title('Operation cost', fontsize=15)
# plt.grid()
plt.xlabel('Training steps (K steps)', fontsize=12)
plt.ylabel('Operation cost', fontsize=12)
plt.subplot(3, 3, 6)
