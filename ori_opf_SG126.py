...
class traditional_solver:
    ...
    def play_game(self, start_idx, epi_idx, expert_uc=None, opt_name=None, ori_renewable_max=None):
        ...
        while not done and step < 288:
            ...
            redundency_adjust = -(1 - np.sign(balance_up_redundency)) / 2 * (balance_up_redundency - 20) + \
                                (1 - np.sign(balance_down_redundency)) / 2 * (balance_down_redundency - 20)
            delta = sum(observation.nextstep_load_p) - sum(observation.load_p) - \
                    sum(adjust_gen_p) + \
                    redundency_adjust
            ...
            upgrade_redundency = (action_high - adjust_gen_p) * mask
            downgrade_redundency = (adjust_gen_p - action_low) * mask
            modification = (1 + np.sign(delta)) / 2 * \
                           (delta / upgrade_redundency.sum()).clip(-0.999, 0.999) * upgrade_redundency + \
                           (1 - np.sign(delta)) / 2 * \
                           (delta / downgrade_redundency.sum()).clip(-0.999, 0.999) * downgrade_redundency
            change_mask = (np.abs(delta) > 30).astype(np.float32)
            adjust_gen_p += modification * change_mask
