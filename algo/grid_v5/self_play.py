...
class GameHistory:
    ...
    def game_over(self):
        ...
        self.reward_history = np.array(self.reward_history).astype(np.float32)
    ...
    def get_stacked_attacker_flag(self, index, num_stacked_observations):
        """
        Generate a new observation with the observation at the index position
        and num_stacked_observations past observations and actions stacked.
        """
        ...
        for past_observation_index in reversed(
            ...
            if 0 <= past_observation_index:
                attacker_flags.append(self.attacker_flag_history[past_observation_index])
            else:
                attacker_flags.append(self.attacker_flag_history[0])

        return np.array(attacker_flags)


    def get_stacked_reward(self, index, num_stacked_observations):
        """
        Generate a new observation with the observation at the index position
        and num_stacked_observations past observations and actions stacked.
        """
        ...
        for past_observation_index in reversed(
            ...
            if 0 <= past_observation_index:
                rewards.append(self.reward_history[past_observation_index])
            else:
                rewards.append(self.reward_history[0])
...
class ExpertPlay:
    def __init__(self, id, config, seed, shared_storage, replay_buffer):
        ...
        self.envs = [config.new_game(seed+i,
                                     ...
                                     is_test=False,
                                     # two_player=config.add_attacker,
    ...
    def play_expert_games(self, trained_steps=0):
        ...
        is_attackers = [False for _ in range(self.config.expert_n_parallel)]
        ...
        self.traditional_solver = traditional_solver(self.config, observations[0], dc_opf_flag=True,
                                                     unit_comb_flag=True, for_training_flag=True)
        ...
        for i, game_history in enumerate(game_histories):
            ...
            random_action = self.config.sample_random_actions(1, is_attacker=False).reshape(-1)
            game_history.action_history.append(random_action)
            game_history.attacker_action_history.append(self.config.sample_random_actions(1, is_attacker=True).reshape(-1))
            game_history.observation_history.append(state_norm)
            game_history.origin_state_history.append(state)
            game_history.attacker_flag_history.append(is_attackers[i])
            ...
            game_history.priorities.append(0.0)
        ...
        while sum(dones) < self.expert_n_parallel and steps < self.config.max_moves:
            ...
            if self.config.use_gumbel:
                root_values, root_distributions, root_actions, root_attacker_actions, ori_values, selected_actions = run_multi_support_adversarial_gumbel(
                    ...
                    is_attackers=np.array([int(is_attacker) for is_attacker in is_attackers]),
                    root_rewards=np.array(root_rewards),
                    temperature=self.config.visit_softmax_temperature_fn(trained_steps)
                )
            else:
                root_values, root_distributions, root_actions, root_attacker_actions, ori_values = run_multi_support_adversarial(
                    observations=states_norm,
                    model=self.model,
                    config=self.config,
                    ready_masks=ready_masks,
            ...
            root_attacker_actions = np.array(root_attacker_actions)
            root_visit_counts = np.array(root_distributions).astype(np.float32)  # [ENV_ID, N_ACTIONS, 1]
            ori_values = np.array(ori_values)
            ...
            for i, game_history in enumerate(game_histories):
                if dones[i]:
                    ...
                    next_line_status.append(observations[i].line_status)
                    root_rewards.append(game_history.reward_history[-1])
                    continue
                ...
                if self.config.use_gumbel:
                    action1 = mcts_action[selected_actions[i]]
                    attacker_action = mcts_attacker_action[selected_actions[i]]
                else:
                ...
                if is_attackers[i]:
                    ...
                    if settings.num_line - sum(line_status[i]) >= self.config.N_k or np.random.random() > self.config.attack_prob:
                        ...
                        attack_cnts[i] = 4
                        for j in range(1, attack_cnts[i]):
                            try:
                                game_history.attack_priorities[-j] = 0.9
                            except:
                                break
                    ...
                    open_id = np.where(action[settings.num_gen - 1:settings.num_gen + len(settings.thermal_ids)] > 0)[0]
                    close_id = np.where(action[settings.num_gen + len(settings.thermal_ids):] > 0)[0]
                    self.Qs[i].append([ori_states[i][0], self.envs[i].last_obs.gen_p[17], action1[:54].sum(),
                                       ...
                                       [] if open_id[0] == len(settings.thermal_ids) else settings.thermal_ids[open_id[0]],
                                       [] if close_id[0] == len(settings.thermal_ids) else settings.thermal_ids[close_id[0]],
                                       is_attackers[i],
                ...
                if attack_cnts[i] > 0:
                    game_history.attack_priorities[-1] = 0.9
                    attack_cnts[i] -= 1

                root_rewards.append(reward)
                next_line_status.append(next_observation.line_status)
                ...
                if done:
                    dones[i] = 1
                    infos[i] = info
                    if info == {'fail_info': 'balance gen out of bound'}:
                        print(f'expert_play fail info={info}')
                        steps_t = len(game_history.action_history)
                ...
                game_history.attacker_action_history.append(attacker_action)
                game_history.observation_history.append(state_norm)
                game_history.origin_state_history.append(state)
                ...
                game_history.priorities.append(np.abs(mcts_value - ori_values[i]) ** self.config.PER_alpha)
            ...
            if self.config.add_attacker:
                is_attackers = [not is_attacker for is_attacker in is_attackers]
        ...
        for i, game_history in enumerate(game_histories):
            ...
            game_history.priorities = np.array(game_history.priorities)
...
class SelfPlay:
    ...
    def play_multi_games(self, trained_steps=0):
        ...
        observations = [env.reset(ori_obs=True) for env in self.envs]
        is_attackers = [False for _ in range(self.config.n_parallel)]
        ...
        while sum(dones) < self.n_parallel and steps < self.config.max_moves:
            ...
            for i, game_history in enumerate(game_histories):
                ...
                if self.config.use_gumbel:
                    action = mcts_action[selected_actions[i]]
                    attacker_action = mcts_attacker_action[selected_actions[i]]
                else:
                ...
                if is_attackers[i]:
                    ...
                    open_id = np.where(action[settings.num_gen-1:settings.num_gen+len(settings.thermal_ids)] > 0)[0]
                    close_id = np.where(action[settings.num_gen + len(settings.thermal_ids):] > 0)[0]
                    if open_id == close_id and open_id[0] != 35:
                        import ipdb
                        ipdb.set_trace()
                        print('error')
                    self.Qs[i].append([ori_states[i][0], self.envs[i].last_obs.gen_p[17], action_true[:54].sum(),
                ...
                if done:
                    ...
                    infos[i] = info
                    if info == {'fail_info': 'balance gen out of bound'}:
                        print(f'self_play fail info={info}')
                        steps_t = len(game_history.action_history)
                        while len(self.Qs[i]) > 0:
                            data1 = self.Qs[i].pop()
                            print(
...
class LowDimTestWorker:
    ...
    def __init__(self, checkpoint, config, seed, shared_storage, replay_buffer):
        ...
        # self.game = config.new_game(seed, is_test=True,
        #                             two_player=False,
        #                             # two_player=True,     # True for no attack happens
        #                             attack_all=config.attack_all
        #                             )
        ...
        self.Qs = [collections.deque(maxlen=10) for _ in range(len(self.test_idxs))]
        self.envs = [config.new_game(seed, is_test=True,
                                     # two_player=False,
                                     two_player=True,    # True for no attack happens
                                     attack_all=config.attack_all
        ...
        while training_step < self.config.training_steps:
            ...
            if training_step // self.config.test_interval >= last_test_index:
                ...
                for i, game_history in enumerate(game_histories):
                    ...
                    voltage_violations += epi_vol_voilations[i]
                    reactive_violations += epi_reac_violations[i]
                    bal_p_violations += epi_bal_p_violations[i]
                    soft_overflows += epi_soft_overflows[i]
                    hard_overflows += epi_hard_overflows[i]
                    running_costs.append(epi_running_cost[i])
                    renewable_consumption_rate.append(epi_renewable_consumption[i])
                ...
                for i, ep in enumerate(self.test_idxs):
                    self.shared_storage.set_info.remote(
                        {
    ...
    def select_action_v2(self, actions, visit_counts, temperature):
        """
      Select action according to the visit count distribution and the temperature.
      The temperature is changed dynamically with the visit_softmax_temperature function
      in the config.
    ...
    #     game_history.action_history.append(self.config.sample_random_actions(1, is_attacker=True).reshape(-1))
    #     game_history.observation_history.append(state_norm)
    #     game_history.origin_state_history.append(state)
    #     game_history.reward_history.append(0)
    #     game_history.reward_true_history.append(0)
    ...
    def play_games_fast(
            self, temperature, train_steps=0
    ):
        """
        Play one game with actions based on the Monte Carlo tree search at each moves.
        """
        voltage_violations, reactive_violations, bal_p_violations, soft_overflows, hard_overflows = \
            [0 for _ in range(len(self.test_idxs))], [0 for _ in range(len(self.test_idxs))], \
            [0 for _ in range(len(self.test_idxs))], [0 for _ in range(len(self.test_idxs))], \
            [0 for _ in range(len(self.test_idxs))]
        running_costs, renewable_cosumption_rates = [[] for _ in range(len(self.test_idxs))], [[] for _ in range(len(self.test_idxs))]
        observations = [env.reset(ori_obs=True, start_sample_idx=self.test_idxs[i]) for i, env in enumerate(self.envs)]
        is_attackers = [False for _ in range(len(self.test_idxs))]

        state_mean = ray.get(self.shared_storage.get_info.remote("state_mean"))
        state_std = ray.get(self.shared_storage.get_info.remote("state_std"))

        game_histories = [GameHistory() for _ in range(len(self.test_idxs))]
        ori_states, ready_masks, closable_masks, action_highs, action_lows = [], [], [], [], []
        line_status = []
        root_rewards = []
        for i, game_history in enumerate(game_histories):
            ...
            state_norm = (state - state_mean) / (state_std + 1e-4)
            action_high, action_low = get_action_space(observations[i], self.config.parameters, settings)
            game_history.action_history.append(self.config.sample_random_actions(1, is_attacker=False).reshape(-1))
            game_history.observation_history.append(state_norm)
            game_history.origin_state_history.append(state)
            game_history.reward_history.append(0)
            game_history.reward_true_history.append(0)
        ...
        infos = [{} for i in range(len(self.test_idxs))]
        states_norm = (ori_states - state_mean) / (state_std + 1e-4)
        ready_masks = np.array(ready_masks)
        closable_masks = np.array(closable_masks)
        action_highs = np.array(action_highs)
        ...
        while sum(dones) < len(self.test_idxs) and steps < self.config.max_moves:
            ...
            for i, game_history in enumerate(game_histories):
                ...
                # action = self.select_action_v2(
                #     mcts_action, mcts_visit_count,
                #     temperature
                # )
                if self.config.use_gumbel:
                    action = mcts_action[selected_actions[i]]
                else:
                    action = self.select_action(mcts_action, mcts_visit_count, temperature)
                ...
                if not self.config.multi_on_off:
                    ...
                    for action_tmp in mcts_action:
                        ...
                        if self.config.parameters['only_power']:
                            ...
                            renewable_consumption = sum((np.asarray(self.envs[i].last_obs.gen_p)+action_tmp_real[:54])[settings.renewable_ids])/sum(np.asarray(self.envs[i].last_obs.nextstep_renewable_gen_p_max))
                        renewable_consumption = ((renewable_consumption*1000)//1)/1000
                        renewable_consumptions.append(renewable_consumption)
                ...
                if gen_reactive_power_reward(next_observation, settings) < 0.0:
                    reactive_violations[i] += 1
                if sub_voltage_reward(next_observation, settings) < -0.006:
                    voltage_violations[i] += 1
                if balanced_gen_reward(next_observation, settings) < 0.0:
                    bal_p_violations[i] += 1

                running_costs[i].append(calc_running_cost_rew(np.asarray(next_observation.gen_p),
                                                               np.asarray(next_observation.gen_status),
                                                               np.asarray(self.envs[i].last_obs.gen_status), is_real=True))
                renewable_cosumption_rates[i].append(sum(np.asarray(next_observation.gen_p)[settings.renewable_ids]) / sum(np.asarray(next_observation.curstep_renewable_gen_p_max)))

                root_rewards.append(reward)
                next_line_status.append(next_observation.line_status)
                state, ready_mask, closable_mask = get_state_from_obs(next_observation, settings,
                ...
                if len(game_history.action_history) % 10 == 0:
                    ...
                    print(f'test_start_idx={self.test_idxs[i]}, step={len(game_history.action_history)}, reward={reward:.3f}, '
                          f'line_outage={len(self.envs[i].last_obs.line_status) - sum(np.array(self.envs[i].last_obs.line_status).astype(np.float32))}')
                    print(
                        f'penetration_rate={sum(np.asarray(self.envs[i].last_obs.gen_p)[settings.renewable_ids]) / sum(np.asarray(self.envs[i].last_obs.curstep_renewable_gen_p_max)):.3f}')
                    print(f'root_sampled_actions_renewable_rates={renewable_consumptions}')
                    print(f'closed_gen_num={54 - sum(self.envs[i].last_obs.gen_status)}')
                    print(f'closable_gen_num={sum(closable_mask[:-1])}')
                    print(f'root visits={mcts_visit_count}')
                    print(f'root_values={mcts_value}')
                    print(
                        f'root_action_std={mcts_action.std(axis=0).mean()}')
                    # print(
                    #     f'line_over_flow_rew={line_over_flow_reward(observation, settings):.3f}, renewable_consumption_rew={renewable_consumption_reward(observation, settings):.3f}, '
                    #     f'bal_gen_rew={balanced_gen_reward(observation, settings):.3f}, '
                    #     f'gen_reactive_rew={gen_reactive_power_reward(observation, settings):.3f}, sub_voltage_rew={sub_voltage_reward(observation, settings):.3f}, '
                    ...
                    #     f'gen_reac_rew_v2={gen_reactive_power_reward_v2(observation, settings):.3f},'
                    #     f'bal_gen_rew_v2={balanced_gen_reward_v2(observation, settings):.3f}')
                    print_log(self.envs[i].last_obs, settings)
                    print('**********************************************************')
        ...
        for i, game_history in enumerate(game_histories):
            print("Test, Len={}, Reward={}, info={}".format(len(game_history.reward_true_history),
                                                      sum(game_history.reward_true_history), infos[i]))
            if self.config.is_plot:
                headers = ('load', 'actual', 'max', 'closed_num', 'closable_num', 'open_action_high', 'open_action_low', 'gen_status')
                path = os.path.join(self.config.results_path, f'data_{self.test_idxs[i]}.csv')
                with open(path, 'w', encoding='utf-8', newline='') as f:
                    write = csv.writer(f)
                    write.writerow(headers)
                    write.writerows(data)
        ...
        return game_histories, voltage_violations, reactive_violations, bal_p_violations, soft_overflows, hard_overflows, \
               ...
               [sum(renewable_cosumption_rates[i]) for i in range(len(self.test_idxs))]