...
class SelfPlay:
    def __init__(self, config, seed, shared_storage, replay_buffer):
        ...
        self.envs = [config.new_game(seed+i, reward_func=config.reward_func) for i in range(config.n_parallel)]
        self.config = config
        ...
        while sum(dones) < self.n_parallel and steps < self.config.max_moves:
            ...
            for i, game_history in enumerate(game_histories):
                ...
                if not self.config.multi_on_off:
                    ...
                    action_true = combine_one_hot_action(state, action, ready_masks[i], closable_masks[i], self.config, action_highs[i], action_lows[i])
                ...
                game_history.running_cost_rewards.append(running_cost_reward(next_observation, self.envs[i].last_obs, settings))
...
class LowDimTestWorker:
                ...
                self.shared_storage.set_info.remote(
                    {
                        ...
                        "test_vol_vio_rate": voltage_violations / (self.config.max_moves * len(start_sample_idx)),
                        "test_reac_vio_rate": reactive_violations / (self.config.max_moves * len(start_sample_idx)),
                        "test_bal_p_vio_rate": bal_p_violations / (self.config.max_moves * len(start_sample_idx)),
                        "test_soft_overflows": soft_overflows / (self.config.max_moves * len(start_sample_idx)),
                        "test_hard_overflows": hard_overflows / (self.config.max_moves * len(start_sample_idx)),
                        "test_running_cost": sum(running_costs) / len(running_costs),
                        "test_renewable_consumption": sum(renewable_consumption_rate) / len(renewable_consumption_rate),
                    }
                )
            else:
                time.sleep(5)
    ...
    def play_game(
        ...
        print(f'test_start_idx={start_sample_idx}')
        ...
        with torch.no_grad():
            while (
                ...
                running_costs.append(calc_running_cost_rew(np.asarray(observation.gen_p),
                                                           ...
                                                           np.asarray(last_observation.gen_status)))
                ...
                if len(game_history.action_history) % 2 == 0:
                    print('----------------------------------------------------------')
                    print(f'step={len(game_history.action_history)}, reward={reward:.3f}, value={root.value():.3f}, '
                ...
                if done:
                    ...
                    while len(self.Q)>0:
                        data1 = self.Q.pop()
                        print(f'step={steps}, delta_load={data1[0]:.3f}, bal_gen_p={data1[1]:.3f}, action_p={data1[2]:.3f}')
                        steps -= 1
                    # import ipdb
                    # ipdb.set_trace()
                state, ready_mask, closable_mask = get_state_from_obs(observation, settings, self.config.parameters)
