...
class Trainer:
    ...
    def loss_imitation(self, expert_policy, expert_action):
        ...
        loss += -(torch.log_softmax(expert_policy[:, 2 * action_dim:2 * action_dim + one_hot_dim], dim=1) *
                  (expert_action[:, action_dim:action_dim + one_hot_dim]+1e-3).clip(0, 1)).sum(1)
        loss += -(torch.log_softmax(expert_policy[:, 2 * action_dim + one_hot_dim:], dim=1) *
                  (expert_action[:, action_dim + one_hot_dim:]+1e-3).clip(0, 1)).sum(1)
        ...
        if self.config.efficient_imitation:
            ...
            pred_value_scalar = (
                torch_utils.support_to_scalar(value[:batch_size // 2],
                                              self.config.support_size, self.config.value_support_step).detach().cpu().squeeze().numpy()
            )
        ...
        for i in range(1, self.config.num_unroll_steps + 1):
            ...
            if self.config.multi_reward:
                for item, coeff, target_item in zip(reward, self.config.reward_coeffs, multi_reward_targets):
                    reward_loss += coeff * self.loss_reward_fn(item, target_item[:, i:i+1])
            else:
