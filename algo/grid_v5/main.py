...
from algo.grid_v5.self_play import LowDimTestWorker, SelfPlay
...
class RZero:
    ...
    def __init__(self, config=None, test_throughput=False):
        ...
        if config.load_pretrain_model:
            ...
            # self.checkpoint["weights"] = torch.load(config.model_load_path)
    ...
    def train_new(self, log_in_tensorboard):
        ...
        if self.config.imitation:
            self.self_play_workers = [
                LowDimSelfPlayWorker.remote(
                    idx, self.checkpoint, self.config, seed, self.shared_storage, self.replay_buffer_worker
                )
                for idx, seed in enumerate(range(self.config.num_workers))
            ]
        else:
            self.self_play_workers = [
                SelfPlay.remote(self.config, seed, self.shared_storage, self.replay_buffer_worker)
                for idx, seed in enumerate(range(self.config.num_workers))
            ]
        ...
        [
            test_worker.spin.remote() for test_worker in self.test_workers
        ]
