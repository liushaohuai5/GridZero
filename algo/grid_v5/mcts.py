...
class Node:
    ...
    def dump_tree(self):
        ...
        for aid in self.action_ids:
            ...
            attacker_action = self.attacker_actions[aid]
            child = self.children[aid]

            child_info = child.dump_tree()
            tree_info['act'].append(action)
            tree_info['attacker_act'].append(attacker_action)
    ...
    def expand(self, to_play, reward, actions, attacker_actions, hidden_state, policies, child_q=None):
        """
        Expand a given (leaf) node.
        :param reward:          The estimated reward when transit into this state in MCTS.
        :param actions:         The sampled operator actions starting from this state.
        :param attacker_actions:The attacker actions starting from this state.
        :param hidden_state:    The state representation of this state.
        :return: void
        """
        if to_play:
            policies = policies[0]
            # attacker_actions = attacker_actions[0]
            for i, act in enumerate(actions):
                self.action_ids.append(i)
                self.actions[i] = act
                self.attacker_actions[i] = attacker_actions[i]
            ...
            for i, id in enumerate(self.action_ids):
                policy_max = max(policies[i], policy_max)

            policy = [math.exp(policies[i] - policy_max) for i, id in enumerate(self.action_ids)]
            policy_sum = sum(policy)

            # Add children.
            for i, id in enumerate(self.action_ids):
                prior = policy[i] / policy_sum
                self.children[id] = Node(prior=prior)
...
class MCTS:
    ...
    def ucb_score(self, parent, child, min_max_stats, parent_mean_q, is_attacker):
        """
    ...
    def run(self,
        ...
        if not self.config.attack_all:
            sampled_actions1 = np.eye(self.config.attacker_action_dim)
        else:
            num = self.config.mcts_num_policy_samples + self.config.mcts_num_random_samples + self.config.mcts_num_expert_samples
            sampled_actions1 = F.gumbel_softmax(attacker_policy_info.unsqueeze(1).repeat(1, num, 1), hard=True,
                                            dim=-1).detach().cpu().numpy()[0]

        root.expand(to_play=root.to_play, reward=root_reward, actions=sampled_actions,
                    attacker_actions=sampled_actions1, hidden_state=hidden_state,
                    policies=torch_utils.tensor_to_numpy(attacker_policy_info).tolist(), child_q=child_q)
        ...
        for _ in range(self.config.num_simulations):
            ...
            while node.expanded():
                ...
                attacker_action = node.attacker_actions[action_id]
            ...
            # print([node.to_play for node in search_path])

            (
                leaf_value, leaf_reward,
                leaf_policy, leaf_attacker_policy, leaf_hidden_state
            ) = model.recurrent_inference(leaf_parent.hidden_state,
                                          torch_utils.numpy_to_tensor(action.reshape(1, *action.shape)),
                                          torch_utils.numpy_to_tensor(attacker_action.reshape(1, *attacker_action.shape)),
                                          to_plays=[leaf_parent.to_play]
                                          )
            ...
            if not self.config.attack_all:
                leaf_actions_attacker = np.eye(self.config.attacker_action_dim)
            else:
                num = self.config.mcts_num_policy_samples + self.config.mcts_num_random_samples + self.config.mcts_num_expert_samples
                leaf_actions_attacker = F.gumbel_softmax(attacker_policy_info.unsqueeze(1).repeat(1, num, 1), hard=True,
                                                dim=-1).detach().cpu().numpy()[0]

            leaf_actions = leaf_actions_policy

            node.expand(1 - leaf_parent.to_play if self.config.add_attacker else 0, leaf_reward, leaf_actions, leaf_actions_attacker, leaf_hidden_state,
                        torch_utils.tensor_to_numpy(leaf_attacker_policy).tolist())
    ...
    def select_child(self, node, min_max_stats, mean_q):
        ...
        """
        is_attacker = 0 if node.to_play == 0 else 1

        max_ucb = max(
            self.ucb_score(node, child, min_max_stats, mean_q, is_attacker)
        ...
        action = np.random.choice(
            [
                action
                for action, child in node.children.items()
                if self.ucb_score(node, child, min_max_stats, mean_q, is_attacker) == max_ucb
            ]
        )
