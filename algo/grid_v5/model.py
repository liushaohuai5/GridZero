...
class MLPModel(nn.Module):
    def __init__(self, config):
        ...
        # self.expert_pi = mlp(self.hidden_shape, self.)
    ...
    def sample_mixed_actions(self, policy, config, is_root,
        ...
        if is_root:
            ...
            if config.explore_type == 'add':
                ...
                sigma = torch.exp(log_std)
                # distr = torch.distributions.Normal(mu, sigma+1e-4)
                ...
                if not is_test:
                    ...
                    policy_open_one_hots_real, policy_close_one_hots_real = self.determine_open_close_one_hot(policy, n_policy_action, modified_action[:, :n_policy_action, :], ready_mask, closable_mask, add_explore_noise=False, is_root=is_root, ori_states=state)
                    policy_open_one_hots_random, policy_close_one_hots_random = self.determine_open_close_one_hot(policy, n_random_action, modified_action[:, -n_random_action:, :], ready_mask, closable_mask, add_explore_noise=True, is_root=is_root, ori_states=state)
                    ...
                    policy_open_one_hots, policy_close_one_hots = self.determine_open_close_one_hot(policy, n_policy_action+n_random_action, modified_action,
                                                                                                      ready_mask,
                                                                                                      closable_mask,
            ...
            policy_action = policy_action.clip(-0.999, 0.999)
            # policy_action = torch.nn.functional.tanh(policy_action)
            # policy_action = torch.clamp(policy_action, -1, 1)
            policy_action = torch_utils.tensor_to_numpy(policy_action.permute(1, 0, 2))

            policy_open_one_hots, policy_close_one_hots = self.determine_open_close_one_hot(policy, n_policy_action+n_random_action, add_explore_noise=False, is_root=is_root)
    ...
    def initial_inference(self, observation,
                          ready_mask=None, closable_mask=None, action_high=None, action_low=None,
                          real_flag=False
                          ):
        h = self.encode(observation)
        pi = self.policy_no_split(h)
        if real_flag:
            pi[:, :pi.shape[1]//2] = F.tanh(pi[:, :pi.shape[1]//2])
            # pi = F.tanh(pi)
            pi = self.modify_policy_v2(pi, ready_mask, closable_mask, action_high, action_low)
        value = self.value(h)
    ...
    # @profile
    def determine_open_close_one_hot(self, policy, num, modified_action=None, ready_masks=None, closable_masks=None, add_explore_noise=False, is_root=False, ori_states=None):
        generator_num = self.config.generator_num
        one_hot_dim = self.config.one_hot_dim
        ...
        if ori_states is not None:
            ...
            for i in range(gen_p.shape[1]):
                if i not in settings.renewable_ids:
                    running_cost -= settings.second_order_cost[i] * (expand_gen_p[:, :, i] + modified_action[:, :, i]) ** 2 + \
                                    settings.first_order_cost[i] * (expand_gen_p[:, :, i] + modified_action[:, :, i]) + settings.constant_cost[i]
            running_cost /= 1e5
    ...
    def check_balance_v2(self, state, sampled_actions, action_high, action_low, ready_mask, closable_mask, norm_action=True):
        ...
        addition = 20
        tmp = 20
        redundency_adjust = -(1 - np.sign(balance_up_redundency - addition)) / 2 * (balance_up_redundency - addition - tmp * random.random()) + \
                            (1 - np.sign(balance_down_redundency - addition)) / 2 * (balance_down_redundency - addition - tmp * random.random())
        # redundency_adjust = -(1 - np.sign(balance_up_redundency - addition)) / 2 * (balance_up_redundency - addition) + \
        #                     (1 - np.sign(balance_down_redundency - addition)) / 2 * (balance_down_redundency - addition)
        ...
        modified_sampled_actions = modified_sampled_actions.clip(-1, 1)
