...
class LowDimFastReplayBuffer(object):
    ...
    def save_game(self, game_history):
        ...
        if self.config.norm_type == 'mean_std':
            if len(self.observations) < 4 * 1e3 * self.config.replay_buffer_size:
                ...
                for i in range(ori_states.shape[0]):
                    ...
                    self.observations.append(ori_states[i])
                # action = ray.get(game_history.action_history)
                # self.actions.append(action)

        # import ipdb
        # ipdb.set_trace()
        # ori_states = ray.get(game_history.origin_state_history)
        # self.state_max = np.concatenate((ori_states, self.state_max.reshape(1, *self.state_max.shape)), axis=0).max(axis=0)
        # self.state_min = np.concatenate((ori_states, self.state_min.reshape(1, *self.state_min.shape)), axis=0).min(axis=0)
        #
        # ori_actions = ray.get(game_history.action_history)
        # self.action_max = np.concatenate((ori_actions, self.action_max.reshape(1, *self.action_max.shape)), axis=0).max(axis=0)
        # self.action_min = np.concatenate((ori_actions, self.action_min.reshape(1, *self.action_min.shape)), axis=0).min(axis=0)

        # for i in range(len(game_history.origin_state_history)):
        #     self.observations.append(game_history.origin_state_history[i])
        #     self.actions.append(game_history.action_history[i])

        # mean, std = self.calc_mean_std()
        # for i in range(len(game_history.observation_history)):
        #     game_history.observation_history[i] = (game_history.observation_history[i] - mean) / (std + 1e-6)
        ...
        # if self.config.replay_buffer_size * self.config.max_moves < len(self.observations):
        #     del_num = len(self.observations) - self.config.replay_buffer_size * self.config.max_moves
        #     del self.observations[0:del_num]
        ...
        # print(f'save_history_time={time.time()-x:.3f}')
    ...
    def sample_n_games(self, n_games, force_uniform=False):
        ...
        if self.config.PER and not force_uniform:
            ...
            # game_probs = list(self.game_priorities.values())[:]
        ...
        # print('SEL', len(selected_games))

        ret = []
        for game_id in selected_games:
            game = self.buffer[game_id]
            game_pos, pos_prob = self.sample_position(game, False)
            # sub_game = game.subset(max([0, game_pos - self.config.stacked_observations + 1]), 16)
            # if self.config.PER:
            #     # ret.append((game_id, sub_game.to_list(), game_probs[game_id], game_pos, pos_prob))
            #     ret.append((game_id, game, game_probs[game_id], game_pos, pos_prob))
            # else:
            #     # ret.append((game_id, sub_game.to_list(), game_prob_dict.get(game_id), game_pos, pos_prob))
            ret.append((game_id, game, game_prob_dict.get(game_id), game_pos, pos_prob))
        # print(f'sample_time={time.time() - x:.3f}')
        return ret
