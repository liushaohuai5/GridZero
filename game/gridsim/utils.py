...
def calc_running_cost_rew(last_real_gen_p, gen_status=None, last_gen_status=None, is_real=False):
    r_c = 0.0
...
def combine_one_hot_action(state, action_one_hot, ready_thermal_mask, closable_thermal_mask, config, action_high, action_low, is_test=False):
    ...
    redundency_adjust = -(1 - np.sign(balance_up_redundency)) / 2 * (
            balance_up_redundency - 20) + \
                        (1 - np.sign(balance_down_redundency)) / 2 * (
                                balance_down_redundency - 20)
    if config.parameters['only_power']:
...
def get_state_from_obs(obs, settings, parameters):
    if parameters['encoder'] == 'mlp':
        ...
        state.append(np.reshape(np.array(obs.gen_p[17] - (bal_gen_p_mid - 80), dtype=np.float32), (-1,)))   # balance gen_p to its lower bound
        state.append(np.reshape(np.array((bal_gen_p_mid + 80) - obs.gen_p[17], dtype=np.float32), (-1,)))  # balance gen_p to its upper bound
