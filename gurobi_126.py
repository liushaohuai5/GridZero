...
print({
                        "test_episode_length": sum(episode_lengths) / len(episode_lengths),
                        "test_total_reward": sum(scores) / len(scores),
                        "test_vol_vio_rate": voltage_violations / sum(episode_lengths),
                        "test_reac_vio_rate": reactive_violations / sum(episode_lengths),
                        "test_bal_p_vio_rate": bal_p_violations / sum(episode_lengths),
                        "test_soft_overflows": soft_overflows / sum(episode_lengths),
                        "test_hard_overflows": hard_overflows / sum(episode_lengths),
                        "test_running_cost": sum(running_costs) / sum(episode_lengths),
                        "test_renewable_consumption": sum(renewable_consumption_rate) / sum(episode_lengths),
                    })
import ipdb
ipdb.set_trace()
