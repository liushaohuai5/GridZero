...
class Disconnect(object):
    ...
    def random_cut(self, attack_all=False):
        if self.np_random.rand() < self.prob_dis:
            if not attack_all:
                ...
                dis_line_name = self.np_random.choice(self.lnname)
    ...
    def get_disc_name(self, last_obs, attack_all=False, attack=None, two_player=False):
        ...
        if not two_player:
            ...
            if not attack_all:
                dis_line_id = [self.lnname.index(self.white_list[attack])] if attack else []
            else:
                dis_line_id = [self.lnname.index(self.lnname[attack])] if attack else []
