...
class ForecastReader(object):
    def __init__(self, settings, is_test=False):
        def_max_renewable_gen_p = pd.read_csv(settings.max_renewable_gen_p_filepath if not is_test else settings.test_max_renewable_gen_p_filepath)
        self.max_renewable_gen_p_all = def_max_renewable_gen_p.values.tolist()
        def_load_p = pd.read_csv(settings.load_p_filepath if not is_test else settings.test_load_p_filepath)
