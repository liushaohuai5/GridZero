...
import mcts_tree_gumbel.cytree as tree_v2
import torch_utils
import ray
...
def run_multi_support_adversarial(observations, model, config,
    ...
    with torch.no_grad():
        ...
        ori_values, root_reward_pred, policy_info, attacker_policy_info, roots_hidden_state = \
        ...
        ori_values = torch_utils.support_to_scalar(ori_values, config.support_size, config.value_support_step)
        ori_values = ori_values.reshape(-1).tolist()
    ...
    return roots.get_values(), roots.get_distributions(), \
           actions_pool[0], attacker_actions_pool[0], ori_values


def run_multi_support_adversarial_gumbel(observations, model, config,
                                         ready_masks=None, closable_masks=None,
                                         action_highs=None, action_lows=None, origin_states=None,
                                         train_steps=0, is_attackers=None, is_test=False, root_rewards=None,
                                         temperature=1.0
    ...
    with torch.no_grad():
        ...
        if not config.attack_all:
            ...
            action_temp1 = F.gumbel_softmax(attacker_policy_info.unsqueeze(1).repeat(1, n_total_actions, 1), hard=True, dim=-1).detach().cpu().numpy()
        ...
        roots = tree_v2.Roots(root_num, n_total_actions, config.n_sim)

        # During preparing, set Q_init.
        to_plays = is_attackers
        uniform_policy = [
            [0.0 for _ in range(n_total_actions)]
            for _ in range(root_num)
        ]
        roots.prepare(root_reward.reshape(-1).tolist(),
                      uniform_policy,
                      m, n_sim,
                      ori_values,
                      torch_utils.tensor_to_numpy(attacker_policy_info).tolist(),
        ...
        gumbels = np.random.gumbel(
            0, 1, (root_num, n_total_actions)
        ) * temperature
        if is_test:
            gumbels = np.zeros_like(gumbels)
        gumbels = gumbels.tolist()

        min_max_stats_lst = tree_v2.MinMaxStatsList(root_num)
        min_max_stats_lst.set_delta(0.01)

        is_reset_lst = [1 for _ in range(root_num)]

        for index_simulation in range(n_sim):
            hidden_states = []
            selected_actions = []
            selected_attacker_actions = []
            results = tree_v2.ResultsWrapper(root_num)
            data_idxes_0, data_idxes_1, last_actions, to_plays = \
                tree_v2.multi_traverse(roots, c_visit, c_scale, discount, min_max_stats_lst, results, index_simulation, gumbels)

            ptr = 0
            for idx_0, idx_1 in zip(data_idxes_0, data_idxes_1):
                try:
                    hidden_states.append(hidden_states_pool[idx_0][idx_1])
                except:
                    import ipdb
                    ipdb.set_trace()
                    print('flase')
                selected_actions.append(actions_pool[idx_0][idx_1][last_actions[ptr]])
                selected_attacker_actions.append(attacker_actions_pool[idx_0][idx_1][last_actions[ptr]])
                ptr += 1
            ...
            # Back-propagate the reward information.
            tree_v2.multi_back_propagate(hidden_state_idx_1, discount, leaves_reward,
                                         leaves_value, uniform_policy,
                                         min_max_stats_lst, results,
                                         is_reset_lst, index_simulation, gumbels, c_visit, c_scale,
                                         torch_utils.tensor_to_numpy(leaves_attacker_policy).tolist(),
                                         to_plays,
                                         int(config.add_attacker))


    return roots.get_values(), roots.get_pi_primes(min_max_stats_lst, c_visit, c_scale, discount), \
           actions_pool[0], attacker_actions_pool[0], ori_values, \
           roots.get_actions(min_max_stats_lst, c_visit, c_scale, gumbels, discount)