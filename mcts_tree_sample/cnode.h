...
namespace tree {
    ...
    class CNode {
        public:
            ...
            void expand_attacker(float reward, int data_idx_0, int data_idx_1, const std::vector<float> &policy_logits);
    ...
    class CRoots{
        public:
            ...
            void prepare(float root_exploration_fraction, const std::vector<std::vector<float>> &noises,
                         ...
                         const std::vector<std::vector<float>> &policies, const std::vector<int> &to_plays);
//            void prepare_attacker(float root_exploration_fraction, const std::vector<std::vector<float>> &noises,
//                                  const std::vector<float> &reward_sums, const std::vector<std::vector<float>> &policies);
            void prepare_no_noise(const std::vector<float> &reward_sums,
                                  const std::vector<std::vector<float>>& q_inits,
                                  const std::vector<std::vector<float>> &policies, const std::vector<int> &to_plays);
//            void prepare_attacker_no_noise(const std::vector<float> &reward_sums, const std::vector<std::vector<float>> &policies);
            void clear();
    ...
    void cmulti_back_propagate(int data_index_1, float discount, const std::vector<float> &rewards, const std::vector<float> &values, tools::CMinMaxStatsList *min_max_stats_lst, CSearchResults &results,
                                const std::vector<int> &to_plays, const std::vector<std::vector<float>> &policies, int add_attacker);
    int cselect_child(CNode* root, tools::CMinMaxStats &min_max_stats, int pb_c_base, float pb_c_init, float discount, float mean_q);
    float cucb_score(CNode *child, tools::CMinMaxStats &min_max_stats, float total_children_visit_counts, float parent_mean_q, float pb_c_base, float pb_c_init, float discount, int is_attacker);
