...
cdef extern from "cnode.h" namespace "tree":
    cdef cppclass CNode:
        ...
        void expand_attacker(float reward_sums, int data_idx_0, int data_idx_1, vector[float] policies)
    ...
    cdef cppclass CRoots:
        ...
        void prepare(float root_exploration_fraction, const vector[vector[float]] &noises,
                     ...
                     const vector[vector[float]] &policies, const vector[int] &to_plays)

        # void prepare_attacker(float root_exploration_fraction, const vector[vector[float]] &noises,
        #                       const vector[float] &reward_sums, const vector[vector[float]] &policies)

        void prepare_no_noise(const vector[float] &reward_sums,
                              const vector[vector[float]] &q_inits,
                              const vector[vector[float]] &policies,
                              const vector[int] &to_plays)

        # void prepare_attacker_no_noise(const vector[float] &reward_sums,
        #                                const vector[vector[float]] &policies)
    ...
    void cmulti_back_propagate(int data_index_1, float discount, vector[float] rewards, vector[float] values,
                               CMinMaxStatsList *min_max_stats_lst, CSearchResults &results, vector[int] &to_plays, vector[vector[float]] &policies, int add_attacker)
